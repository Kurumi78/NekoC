    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (NekoC.isNeko(Bukkit.getPlayer(args[0])) && sender instanceof Player) {
            ...
            if (NekoC.globalCommands) {
                Bukkit.broadcastMessage(ChatColor.GREEN + "[NekoC] " + ChatColor.YELLOW + p.getDisplayName() + ChatColor.LIGHT_PURPLE + " has scratched behind the ear of " + player.getDisplayName());
            } else {
